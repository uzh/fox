...
case class InferencerConfig(
  ...
  lazyThreshold: Option[Double] = Some(1e-9), // Absolute threshold for lazy vertices.
  implicitZero: Boolean = true, // Flag that determiens if zero messages should be sent implicitly.
