...
The most important part of modeling a domain are the rules, which are Horn rules with disjunctive heads, i.e. rules in the form 
`B<sup>1</sup>  &&  ...  &&  B<sup>n</sup> => H<sup>1</sup> || ... || H<sup>m</sup>`
where `H<sup>i</sup>` for `i=1, ..., m` are literals, `B<sup>j</sup>` for `j=1, ..., n` are atoms and the symbols `&&`, `||` and `=>` represent conjunction, disjunction and implication.
